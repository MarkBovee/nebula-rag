...
public sealed class RagQueryService
{
    ...
    /// <summary>
    /// Executes a hybrid ranking query with BM25 + semantic similarity.
    /// </summary>
    /// <param name="queryText">The search query text.</param>
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>List of hybrid-ranked search results with diagnostic scores.</returns>
    /// <exception cref="RagQueryException">Thrown if query execution fails.</exception>
    public async Task<IReadOnlyList<RagSearchResult>> QueryHybridAsync(
        string queryText,
        int topK,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            "Executing hybrid query (topK={TopK}, mode={Mode}, preferredTags={PreferredTags}): {QueryText}",
            topK,
            mode,
            preferredTags?.Count,
            queryText);
        ...
        try
        {
            ...
            var embedding = _embeddingGenerator.GenerateEmbedding(queryText, _settings.Ingestion.VectorDimensions);
            var results = await _store.SearchHybridAsync(
                embedding,
                queryText,
                topK,
                preset,
            ...
            if (results.Count == 0)
            {
                _logger.LogInformation("Hybrid query returned no results ({ElapsedMs}ms)", timer.ElapsedMilliseconds);
            }
            else
            {
                _logger.LogInformation("Hybrid query returned {ResultCount} results ({ElapsedMs}ms, top score: {TopScore:F4})",
                    results.Count,
                    timer.ElapsedMilliseconds,
                    results[0].Score);
            }
            ...
            if (timer.ElapsedMilliseconds > 1000)
            {
                _logger.LogWarning("Slow hybrid query detected ({ElapsedMs}ms)", timer.ElapsedMilliseconds);
            }
        ...
        {
            _logger.LogError(ex, "Hybrid query execution failed: {QueryText}", queryText);
            throw new RagQueryException($"Hybrid query execution failed: {queryText}", ex);
        }
    }
}
